...
\begin{proof}
    ...
    following directly from the linearity properties of the inner product. For each $j = 2,\ldots,m$, define
        ...
        \begin{claim}
            ...
            \begin{proof}
                For the base case, notice that $\ip{v_1}{v_2} = 0$. Assume now that whenever $1 \leq i < j < k$, $\ip{v_i}{v_j}
            \end{proof}
        \end{claim}

    Now define $e_j = \frac{1}{\norm[v_j]}v_j$ for each $j = 1,\ldots, m$. Then each $e_j$ has norm 1, the list $e_1,\ldots, e_m$ is pair-wise orthogonal, and is linearly independent.
\end{proof}
